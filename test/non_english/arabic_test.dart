...
void main() {
  group('Arabic parsing', () {
    test('parse Arabic verse reference', () {
      final ref =
          verseRefFromStr('يوحنا 3:16', language: BibleLanguageEnum.arabic);
    ...
    test('parse Arabic verse range', () {
      final ref = verseRangeRefFromStr('يوحنا 3:16-17',
    ...
    for (final book in BibleBookEnum.values) {
      test('parse Arabic book name: ${book.name}', () {
        ...
        expect(names, isNotEmpty,
            reason: 'Missing Arabic book names for ${book.name}');
    ...
    for (final book
        in BibleBookEnum.values.where((b) => !skipCollisions.contains(b))) {
      test('parse Arabic book abbreviations: ${book.name}', () {
        ...
        expect(abbreviations, isNotEmpty,
            reason: 'Missing Arabic book abbreviations for ${book.name}');
    ...
    test('Arabic collision handling - Mark vs Lamentations', () {
      ...
      final ref1 =
          verseRefFromStr('مر 1:1', language: BibleLanguageEnum.arabic);
      expect(
          [BibleBookEnum.mark, BibleBookEnum.lamentations].contains(ref1.book),
          isTrue);
    });
    ...
    test('Arabic collision handling - John vs Joel', () {
      ...
      final ref1 =
          verseRefFromStr('يو 1:1', language: BibleLanguageEnum.arabic);
      expect(
          [BibleBookEnum.john, BibleBookEnum.joel].contains(ref1.book), isTrue);
    });
  });
}
